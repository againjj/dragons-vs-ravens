...
- MTG game implementation code must preserve object identity when nothing changes: if a rule/action leaves a model value unchanged, reuse the existing state object, lists, and other built-in objects rather than constructing equivalent replacements.
...
- MTG code must be condition covered by tests. Every condition introduced in MTG production code should have tests that exercise both the true branch and the false branch, including validation guards and no-op identity-preservation paths.
