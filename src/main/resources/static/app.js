...
const captureSkipButton = document.querySelector("#capture-skip-button");
const columnLabelsTop = document.querySelector("#column-labels-top");
const columnLabelsBottom = document.querySelector("#column-labels-bottom");
const rowLabelsLeft = document.querySelector("#row-labels-left");
...
const getSquareName = (rowIndex, colIndex) => `${rowLetters[rowIndex]}${colIndex + 1}`;
...
const oppositeSide = (side) => (side === "dragons" ? "ravens" : "dragons");

const sideOwnsPiece = (side, piece) => {
    if (piece === "gold") {
        return side === "dragons";
    }
...
const canCapturePiece = (side, piece) => (side === "dragons" ? piece === "raven" : piece === "dragon");

const cycleSetupPiece = (square) => {
    if (square === "e5") {
        return;
    }
...
const commitTurn = (capturedSquare) => {
...
const handleMove = (origin, destination) => {
    ...
    const hasCapturablePiece = [...state.board.values()].some((boardPiece) =>
...
const moveToNotation = (move) => `${move.from}-${move.to}${move.captured ? `x${move.captured}` : ""}`;
...
const renderBoard = () => {
    ...
    const targetableSquares = new Set();
...
const renderMoveList = () => {
    moveListElement.innerHTML = state.turns.map((move) => `<li>${moveToNotation(move)}</li>`).join("");
};
