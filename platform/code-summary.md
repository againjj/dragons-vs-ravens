...
- `platform/backend/src/main/kotlin/com/ayaziangames/platform/game/runtime/*.kt`
  ...
  - Exposes registered game module identities, public unfinished game listings, and signed-in player-game listings/streams, with shared sorting for both list surfaces.
...
- Expose the registered module list so the frontend shell can hide game entries that are not enabled on the current server.
